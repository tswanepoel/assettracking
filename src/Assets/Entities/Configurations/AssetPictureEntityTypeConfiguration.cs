...
namespace Assets.Entities.Configurations
{
    public class AssetPictureEntityTypeConfiguration : IEntityTypeConfiguration<AssetPicture>
    {
        public void Configure(EntityTypeBuilder<AssetPicture> builder)
        {
            ...
            builder.HasOne(x => x.Asset).WithMany(x => x.Pictures).HasForeignKey(x => x.AssetId).HasConstraintName("FK_AssetPicture_Asset");
            builder.HasOne(x => x.Picture).WithMany().HasForeignKey(x => x.AssetId).HasConstraintName("FK_AssetPicture_Blob");
        }
    }
}
