...
namespace Assets.Entities.Configurations
{
    public class ContactEntityTypeConfiguration : IEntityTypeConfiguration<Contact>
    {
        public void Configure(EntityTypeBuilder<Contact> builder)
        {
            ...
            builder.HasOne(x => x.Tenant).WithMany(x => x.Contacts).HasForeignKey(x => x.TenantId).HasConstraintName("FK_Contact_Tenant");
            builder.HasOne(x => x.ContactType).WithMany(x => x.Contacts).HasForeignKey(x => x.ContactTypeId).HasConstraintName("FK_Contact_ContactType");
            builder.HasIndex(x => new { x.TenantId, x.Guid }).IsUnique().HasName("IX_Contact_Tenant_Guid");
        }
    }
}
