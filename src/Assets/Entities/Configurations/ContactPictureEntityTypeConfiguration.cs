...
namespace Assets.Entities.Configurations
{
    public class ContactPictureEntityTypeConfiguration : IEntityTypeConfiguration<ContactPicture>
    {
        public void Configure(EntityTypeBuilder<ContactPicture> builder)
        {
            ...
            builder.HasKey(x => new { x.ContactId, x.PictureId }).HasName("PK_ContactPicture").ForSqlServerIsClustered();
            ...
            builder.HasOne(x => x.Contact).WithMany(x => x.Pictures).HasForeignKey(x => x.ContactId).HasConstraintName("FK_ContactPicture_Contact");
            builder.HasOne(x => x.Picture).WithMany().HasForeignKey(x => x.ContactId).HasConstraintName("FK_ContactPicture_Blob");
        }
    }
}
